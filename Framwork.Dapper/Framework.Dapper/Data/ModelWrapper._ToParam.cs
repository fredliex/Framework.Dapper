...
#pragma warning restore 618
                    ...
                    if (!memberType.IsValueType || (underlyingType = Nullable.GetUnderlyingType(memberType)) != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Reflect.Dapper.EmitInt32(il, 0);
                                il.Emit(setLocOpCodes[varIndexSize]);
                            }
                            //06. 跳到nullHandleDone
                            nullHandleDone = il.DefineLabel();
                            il.Emit(OpCodes.Br_S, nullHandleDone.Value);
                    ...
                    if (memberType.IsValueType)
                    {
                        ...
                        {
                            il.EmitCall(OpCodes.Call, memberType.GetMethod(nameof(Nullable<int>.GetValueOrDefault), Type.EmptyTypes), null); // stack is [parameters] [parameters] [parameter] [parameter] [typed-value]
                    ...
                    else
                    {
                        ...
                        else if (dbType == DbType.String || dbType == DbType.AnsiString)
                        {
                            ...
                            Reflect.Dapper.EmitInt32(il, -1); // stack is [parameters] [parameters] [parameter] [parameter] [typed-value] [-1]
                            il.MarkLabel(lenDone);
                            il.Emit(setLocOpCodes[varIndexSize]); // stack is [parameters] [parameters] [parameter] [parameter] [typed-value]
                        }
                    }
...
#pragma warning restore 618
                    ...
                    else
                    {
                        ...
                        if (dbType == DbType.String || dbType == DbType.AnsiString)
                        {
                            var endOfSize = il.DefineLabel();
                            // don't set if 0
                            il.Emit(getLocOpCodes[varIndexSize]); // [parameters] [parameters] [parameter] [size]
                            il.Emit(OpCodes.Brfalse_S, endOfSize); // [parameters] [parameters] [parameter]

                            il.Emit(OpCodes.Dup);// stack is now [parameters] [parameters] [parameter] [parameter]
                            il.Emit(getLocOpCodes[varIndexSize]); // stack is now [parameters] [parameters] [parameter] [parameter] [size]
                            il.EmitCall(OpCodes.Callvirt, Reflect.IDbDataParameter_Size_Set, null); // stack is now [parameters] [parameters] [parameter]
