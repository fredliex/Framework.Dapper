...
{
    internal static partial class DbWrapperHelper
    {
        ...
        private static ConcurrentDictionary<DbProviderFactory, DbProviderFactory> cacheDbProviderFactoryWrapper = new ConcurrentDictionary<DbProviderFactory, DbProviderFactory>();

        internal static DbProviderFactory Wrap(DbProviderFactory factory) => cacheDbProviderFactoryWrapper.GetOrAdd(factory, 
            f => (DbProviderFactory)typeof(WrapperBuilder<>).MakeGenericType(f.GetType()).GetMethod(nameof(WrapperBuilder<DbProviderFactory>.WrapDbProviderFactory)).Invoke(null, new object[] { f })
        );
            ...
            while (true)
            {
                ...
                foreach (var instanceMethod in curType.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly))
                {
                    ...
                    if (!existMethods.ContainsKey(signature))
                    {
                        /*
                        Debug.WriteLine("{0}\t ---> \t{1}",
                            string.Format("{0}\t{1}\t{2}({3})", attributes.ToString().Replace(" ", ""), n.DeclaringType.Name, n.Name, string.Join(",", n.GetParameters().Select(p => p.ParameterType.Name)), n.ReflectedType.Name),
